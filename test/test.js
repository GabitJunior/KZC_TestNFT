...
describe("NFT", function () {
  ...
  it("should allow mint last NFT", async function () {
   ...
   it("should not allow mint more than 10 NFTs of the same type", async function () {
     // Минтим 10 NFT одного типа (0 - й токен)
   ...
   it("should return the correct token URI for all token", async function () {
     ...
     for (let i = 0; i < maxSupply; i++) {
        ...
        expect(tokenURI).to.equal(
          "ipfs://QmbuFxymcSsQvx3AoGVE1hMaW5mPSoM9SAefBXDYVqjzQr/" + i + ".json"
        );
     }
   });
   ...
   it("should not allow non-owners to change the base URI", async function () {
     const newBaseURI = "ipfs://QmbuFxymcSsQvx3AoGVE1hMaW5mPSoM9SAefBXDYVqjzQr/";
     await expect(
   ...
   it("should be changed by owner", async function () {
     ...
     await contract.tokenURI(0).then((result) => {
       expect(result).to.be.eq(
         "ipfs://QmbuFxymcSsQvx3AoGVE1hMaW5mPSoM9SAefBXDYVqjzQr/0.json"
       );
     });
   });
});